...
class QCAS_batch_file():
    ...
    def replace_qcas_command(self, entrylist):
        if len(entrylist) == 2: # Must be two entries.
            with open(os.path.join(OUTPUT_DIR,'qcas_autogenerated.bat'), 'w+') as batch_file:
...
class QCAS_TSL_Generator:
    ...
    def update_fname(self, fname): 
        # split and read year month and day. 
        s = os.path.splitext(fname)[0] 
        entries = s.split('_')
        
        # enstries = ['qcas', '2021', '12', 'v01']
        version = int(entries[3][1:])
        month = int(entries[2])
        year = int(entries[1])

        # just inc version for now. 
        version = version + 1
        output_fname = ''
        if version < 10: 
            output_fname = "qcas_"+str(year)+"_"+str(month)+"_v0"+str(version)+".tsl"
        else: 
            output_fname = "qcas_"+str(year)+"_"+str(month)+"_v"+str(version)+".tsl"

        return output_fname

    def clean_tab_delimited_file(self): 
        fname = self.tab_delimited_filename_tf.get()
        fname_cleaned = os.path.splitext(os.path.basename(fname))[0] + "_cleaned.txt" # without .txt
        with open(fname,'r') as file, open(os.path.join(OUTPUT_DIR, fname_cleaned),'w+') as outfile:
    ...
    def handleButtonPress(self, myButtonPress):
        if myButtonPress == '__tab_delimited_file__':
            ...
            if tmp:
                ...
                # new TSL filename auto update
                self.v.set(self.update_fname(os.path.basename(filename2)))


        elif myButtonPress == '__start__':

            if (os.path.isfile(self.tab_delimited_filename_tf.get())) and (os.path.isfile(self.current_tsl_filename_tf.get())):
                fname, ext = os.path.splitext(os.path.basename(self.tab_delimited_filename_tf.get()))
                if ext == '.txt': 
                    # processing TAB delimited file
                    self.new_fname = self.clean_tab_delimited_file()
                    # update text field
                    # self.current_tsl_filename_tf.delete(0, END)
                    ...
                    messagebox.showinfo("Unsupported .xls", "Does not support reading .xls file at the moment, please export this file as a TAB delimited .txt file")                    
                else: 
                    messagebox.showerror("Cannot process: ", self.tab_delimited_filename_tf.get(), "Please select files first")
            else:
    ...
    def write_game_list_to_file(self, game_list):
        with open(os.path.join(OUTPUT_DIR,self.new_tsl_filename_tf.get()), 'w+') as outfile:
    ...
    def setup_GUI(self):
        ...
        self.v = StringVar()    
        self.v.set("qcas_2021_12_v01.tsl")
        ...
        # debug hack
        if getpass.getuser() == 'aceretjr': 
            current_tsl = 'C:/Users/aceretjr/DJAG/OneDrive Work - Documents/WFH/dev/qcasTSLgenerator/test/qcas_2021_11_v01.tsl'
            self.current_tsl_filename_tf.delete(0, END)
            self.current_tsl_filename_tf.insert(0, current_tsl)

            tab_delimited_fname = 'C:/Users/aceretjr/DJAG/OneDrive Work - Documents/WFH/dev/qcasTSLgenerator/test/20211208 - Tech Services Item List.txt'
            self.tab_delimited_filename_tf.delete(0, END)
            self.tab_delimited_filename_tf.insert(0, tab_delimited_fname)

            self.v.set(self.update_fname(os.path.basename(current_tsl))) # new tsl filename
    ...
    def genNewTSLEntries(self):
        ...
        try:
            self.new_fname
            with open(os.path.join(OUTPUT_DIR, self.new_fname), 'r', encoding='utf-8') as infile:             
            #with open(self.tab_delimited_filename_tf.get(), 'r', encoding='utf-8') as infile: 
                ...
                for row in reader:
                    ...
                    if row['bin_type'] == 'BIN LINK FILE':
                        ...
                        print('Unknown binimage type %s' % row['bin_type'])         
